...
class SourceTermSolver:
    ...
    def compute_heat_flux_xi(
        ...
        r"""Computes the heat flux in axis direction.
    ...
    def compute_viscous_flux_xi(
        ...
        # First calculate velocity gradient and shear rate
        velocity = primitives[self.s_velocity]
        velocity_gradient = self.compute_velocity_gradient(velocity, axis)

        velocity_grad = self.compute_velocity_gradient_cc_no_slice(velocity)
        gamma_dot = self.compute_shear_rate(velocity_grad)
        gamma_dot_at_cf = self.reconstruct_stencil_ui.reconstruct_xi(gamma_dot, axis)

        # print('gamma_dot shape:',gamma_dot.shape)
        # print('temperature shape:',temperature.shape)

        # Then use gamma_dot to get dynamic viscosity
        ...
        # print('gamma_dot cf shape:',gamma_dot_at_cf.shape)
        # print('temperature cf shape:',temperature_at_cf.shape)
    ...
    def compute_viscous_term_xi(
        ...
        # print('gamma_dot', gamma_dot.shape)

        # Then use gamma_dot to get dynamic viscosity
        dynamic_viscosity = self.material_manager.get_dynamic_viscosity(
            temperature, primitives, gamma_dot)
        bulk_viscosity = self.material_manager.get_bulk_viscosity(
    ...
    def compute_velocity_gradient(
        ...
        r"""Computes the velocity gradient at cell faces
        in axis direction.
        ...
        #print('velocity gradient shape:', velocity_gradient.shape)
        return velocity_gradient
        
    def compute_velocity_gradient_no_slice(
            self,
            velocity: Array,
            axis: int
            ) -> Array:
        ...
        #print('velocity gradient shape:', velocity_gradient.shape)
        pad_width = [
            (0, 0),  # no padding for axis 0 (3)
            (0, 0),  # no padding for axis 1 (3)
            (4, 4),  # pad to 12 along axis 2 (4 -> 12)
            (4, 4),  # pad to 12 along axis 3 (4 -> 12)
            (0, 0)   # no padding for axis 4 (1)
        ]

        velocity_gradient_padded = jnp.pad(velocity_gradient, pad_width, mode='constant')
        return velocity_gradient_padded
    ...
    def compute_velocity_gradient_cc_no_slice(self, velocity: Array) -> Array:
        """
        Computes velocity gradient at cell centers without slicing.
    ...
    def compute_shear_rate(self, velocity_gradient: Array) -> Array:
        """
