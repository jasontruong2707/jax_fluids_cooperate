...
class SpatialStencil(ABC):
    ...
    def __init__(self, nh: int, inactive_axes: List, offset: int = 0, use_slicing: bool = True) -> None:
    ...
    def array_slices(self, idx_ranges: List[Tuple[int]], at_cell_center: bool = False) -> None:
        ...
        for idx_range in idx_ranges:
            ...
            for k in idx_range:
                ...
                nhi = -self.n + k + 1 - at_cell_center if self.use_slicing else None
                if nhi == 0:
                    nhi = None 
                else:
                    assert_msg = f"Stencil with left-shift {k} is too wide for domain with {self.n} halo cells."
